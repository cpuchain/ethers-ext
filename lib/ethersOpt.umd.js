...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ethersOpt"] = factory();
	else
		root["ethersOpt"] = factory();
})(self, () => {
...
__webpack_require__.d(__webpack_exports__, {
  ...
  EnsResolver: () => (/* reexport */ EnsResolver),
  ...
  chainNames: () => (/* reexport */ chainNames),
  chunk: () => (/* reexport */ chunk),
  compareBlockHashes: () => (/* reexport */ compareBlockHashes),
  concatBytes: () => (/* reexport */ concatBytes),
  createBatchRequest: () => (/* reexport */ createBatchRequest),
  createBlockTags: () => (/* reexport */ createBlockTags),
  crypto: () => (/* reexport */ utils_crypto),
  digest: () => (/* reexport */ digest),
  digestHex: () => (/* reexport */ digestHex),
  ensRegistries: () => (/* reexport */ ensRegistries),
  ensReverseNode: () => (/* reexport */ ensReverseNode),
  ensStaticResolvers: () => (/* reexport */ ensStaticResolvers),
  ensUniversalResolvers: () => (/* reexport */ ensUniversalResolvers),
  ...
  wildcardResolvers: () => (/* reexport */ wildcardResolvers)
});
...
const { Contract: CreateX_factory_Contract, Interface } = ethers;
const _abi = [
  {
    inputs: [
      {
...
class CreateX__factory {
  ...
  static connect(address, runner) {
    return new CreateX_factory_Contract(address, _abi, runner);
  }
}
...
const { getAddress: traceBlock_getAddress } = ethers;
function formatCallTrace(params, txHash, blockParams) {
  return {
    from: params.from ? traceBlock_getAddress(params.from) : "",
    gas: Number(params.gas || 0),
    gasUsed: Number(params.gasUsed || 0),
    to: params.to ? traceBlock_getAddress(params.to) : "",
    input: params.input,
...
const { Transaction, parseUnits, parseEther, ZeroAddress: op_ZeroAddress } = ethers;
...
async function getL1Fee(oracle, tx) {
  const { unsignedSerialized } = Transaction.from({
    ...
    nonce: tx?.nonce || 1e5,
    to: tx?.to instanceof Promise ? await tx?.to : tx?.to || op_ZeroAddress,
    type: tx?.type || 0,
...
async function populateTransaction(signer, tx = {}) {
  ...
  const [chainId, feeData, nonce, balance, l1Fee] = await Promise.all([
    ...
    typeof tx.txCost === "bigint" || !signer.autoValue ? void 0 : (provider.multicall?.getEthBalance || provider.getBalance)(signerAddress),
