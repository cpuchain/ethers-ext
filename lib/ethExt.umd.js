  ...
  async createBatchRequest(inputs, outputFunc, batchSize) {
    ...
    for (const chunks of chunk(inputs, this.concurrencySize * batchSize)) {
      const chunksResult = (await Promise.all(
        chunk(chunks, batchSize).map(async (_inputs, chunkIndex2) => {
          await sleep(40 * chunkIndex2);
          return (async () => {
            let retries = 0;
            let err;
...
class Provider extends ethJsonRpcProvider {
  ...
  constructor(url, network, options) {
    ...
    this.multicall = this.subProvider.then(
      (provider) => Multicall__factory.connect(options?.multicall || MULTICALL_ADDRESS, provider)
    );
  ...
  async _drainCalls() {
    try {
      (await (await this.multicall).aggregate3.staticCall(
        this.multicallQueue.slice(0, this.multicallMaxCount).map(({ request: { to: target, data: callData } }) => {
          return {
            target,
            callData,
